...
| `tools/inspect_sources.py` | Показывает поля источника и что из них распознаётся |
...
| `tests/run_e2e.py` | Проверка всей цепочки на синтетике (77 проверок) |
...
Движок классифицирует каждый источник по пути и **никогда** не выдаёт теневой
контур за метрики платформы:

| Метка | Когда | Влияние на план |
|---|---|---|
| `REAL` | Метрики Instagram по опубликованному контенту | Двигает приоритет форматов, задаёт критерии успеха |
| `SHADOW` | Путь содержит `shadow`, `learning_`, `synthetic`, `simulat`, `mock`, `sample`, `dry_run` и т.п. | **Никакого.** Показывается отдельным разделом как «требует подтверждения» |
| `MIXED` | Часть источников теневая | Считается по слабейшему звену |
| `REAL` по `--trust` | Путь выглядит теневым, но сверка `verify_insights.py` подтвердила реальные media_id | Полноценно участвует; факт override фиксируется в выходном файле |
| `NOT_MEASURED` | Данных нет | Нулями не заменяется |

На теневых данных отчёт открывается строкой `EVIDENCE: SHADOW`, цикл никогда
не объявляется `VERIFIED`, ранжирование бэклога не меняется, а baseline не
подставляется в критерии успеха.
