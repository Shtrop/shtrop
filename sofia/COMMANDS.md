...
| `power_limit_changed` | смягчение не держалось всё окно: за окно измерены разные режимы |
| `power_limit_unsettled` | окно кончилось сразу после перезагрузки или ни один режим ещё не подтверждён: карта на лимите, который наблюдением не доказан — снимается продолжением окна |
| `power_limit_state_migrated` | состояние окна перенесено от прежней версии набора: чем питались набранные до переноса часы, оно не говорит, и доказательство по лимиту набирается заново |
