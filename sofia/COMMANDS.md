...
без `used_memory` — у всех записей стоит `[N/A]`. В этом случае владелец по
имени известен, а его потребление нет, поэтому решение принимается по суммарной
занятости GPU: занято выше порога → `NO_GO` с пометкой, что владельца
установить нельзя. Пустые значения не читаются как «никто ничего не держит».
