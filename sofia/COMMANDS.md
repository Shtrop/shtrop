...
Если и счётчики не дали картины, решение принимается по суммарной занятости
GPU: занято выше порога → `NO_GO` с пометкой, что владельца установить нельзя.
Пустые значения никогда не читаются как «никто ничего не держит».
