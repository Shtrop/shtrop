...
Экземпляры ComfyUI ищутся не по одному порту. Проба только `-ComfyUrl`
показывает один экземпляр; если на карте работает второй, его очередь не видна,
и картина выглядит необъяснимой: очередь пуста, а карта занята. Поэтому
опрашиваются порты, которые слушают сами рендер-процессы. Два ответивших
экземпляра — доказанное нарушение правила одного владельца, а не догадка:

```powershell
.\check_gpu_lease.ps1 -ComfyPorts 8188,8189    # проверить конкретные порты
```
