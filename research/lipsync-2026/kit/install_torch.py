...
def main() -> int:
    ...
    p.add_argument("--with-vision", action="store_true", help="поставить и torchvision")
    ...
    pkgs = [spec] + (["torchvision"] if a.with_vision else [])
    ...
    new_ver, new_cuda = current_torch()   # процесс тот же, модуль мог быть уже импортирован
    print(f"готово: перезапустите проверку — python preflight.py --repo <dir>")
    return 0
